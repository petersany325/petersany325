...
final class TicketRepository
{
    ...
    public static function createAdvanced(
        ...
        try {
            $pdo->prepare(
                'INSERT INTO tickets (user_id, subject, status, contact_name, phone, customer_id, meta_json) VALUES (?,?,?,?,?,?,?)'
            )->execute([
                $userId,
                $subject,
                'open',
    ...
    public static function forUserDetailed(int $userId, int $limit = 10): array
    {
        try {
            $stmt = db()->prepare(
                'SELECT id, subject, status, contact_name, phone, customer_id, created_at FROM tickets WHERE user_id = ? ORDER BY id DESC LIMIT '
                . (int)$limit
            );
            $stmt->execute([$userId]);
            return $stmt->fetchAll();
    ...
    {
        return db()->query("SELECT id, user_id, subject FROM tickets WHERE status IN ('open','answered','waiting') ORDER BY id DESC LIMIT " . (int)$limit)->fetchAll();
    }
    ...
    public static function addAdminReply(int $ticketId, int $adminId, string $text): void
    {
        ...
        self::setStatus($ticketId, 'answered');
    }

    public static function addUserReply(int $ticketId, int $userId, string $text): void
    {
        db()->prepare('INSERT INTO ticket_messages (ticket_id, sender_id, is_admin, text) VALUES (?,?,0,?)')
            ->execute([$ticketId, $userId, $text]);
    }

    public static function setStatus(int $ticketId, string $status): void
    {
        $allowed = array('open', 'answered', 'waiting', 'closed');
        if (!in_array($status, $allowed, true)) {
            return;
        }
        try {
            db()->prepare('UPDATE tickets SET status=?, updated_at=NOW() WHERE id=?')->execute([$status, $ticketId]);
        } catch (\Throwable $e) {
    ...
    {
        self::setStatus($ticketId, 'closed');
    }
}
