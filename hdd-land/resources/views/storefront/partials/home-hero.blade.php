@php
  ...
  // وقتی بنر Revolution زنده است، هیرو آنلاین را نشان نده (جلوگیری از دوبل‌بنر).
  $skipHero = ! empty($skipHero);
  if (! $skipHero && class_exists(\Plugins\ThemeBuilder\src\ThemeConfig::class)) {
    $skipHero = \Plugins\ThemeBuilder\src\ThemeConfig::bannerIsLive();
  }
@endphp
@if(! $skipHero && ! empty($home['hero_enabled']))
<section
