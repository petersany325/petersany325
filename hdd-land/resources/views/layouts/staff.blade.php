...
<head><meta charset="utf-8">
    ...
    <style>
      ...
      .staff-topbar{display:none;align-items:center;justify-content:space-between;gap:.75rem;padding:.65rem .85rem;background:#10141c;color:#fff;position:sticky;top:0;z-index:40}
      ...
      .staff-backdrop{display:none;position:fixed;inset:0;z-index:25;background:rgba(15,23,42,.45)}
      @media(max-width:900px){
        .staff-topbar{display:flex}
        .staff-shell{grid-template-columns:1fr}
        .staff-side{display:none}
        .staff-shell.side-open .staff-side{
          display:flex;position:fixed;top:0;right:0;bottom:0;width:min(300px,88vw);z-index:35;
          box-shadow:-18px 0 40px rgba(0,0,0,.35)
        }
        .staff-shell.side-open .staff-backdrop{display:block}
      }
    </style>
</head>
<body>
...
<div class="staff-shell" id="staffShell">
  <div class="staff-backdrop" id="staffBackdrop" hidden></div>
...
(function(){
  ...
  function close(){ if(shell) shell.classList.remove('side-open'); if(backdrop) backdrop.hidden=true; }
  function open(){ if(shell) shell.classList.add('side-open'); if(backdrop) backdrop.hidden=false; }
  if(btn&&shell){
    btn.addEventListener('click',function(){
      if(shell.classList.contains('side-open')) close(); else open();
    });
  }
  if(backdrop){ backdrop.addEventListener('click', close); }
  document.querySelectorAll('.staff-nav a').forEach(function(a){
