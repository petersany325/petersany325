...
class PortalNav
{
    ...
    public static function staffItems(array $perms, string $path): array
    {
        ...
        return [
            ...
            ['label' => 'کارمند و ویزیتور', 'href' => url('/admin/accounting/staff'), 'show' => ! empty($perms['accounting']) || ! empty($perms['accounting.payroll']), 'active' => str_contains($path, 'accounting/staff')],
            ['label' => 'دسته چک', 'href' => url('/admin/accounting/checkbooks'), 'show' => ! empty($perms['accounting.checks']) || ! empty($perms['accounting']), 'active' => str_contains($path, 'checkbooks')],
            ['label' => 'چک دریافتی', 'href' => url('/admin/accounting/checks/received'), 'show' => ! empty($perms['accounting.checks']) || ! empty($perms['accounting']), 'active' => str_contains($path, 'checks/received')],
            ['label' => 'چک خرج‌شده', 'href' => url('/admin/accounting/checks/spent'), 'show' => ! empty($perms['accounting.checks']) || ! empty($perms['accounting']), 'active' => str_contains($path, 'checks/spent')],
            ['label' => 'چک‌ها', 'href' => $u('accounting/checks'), 'show' => ! empty($perms['accounting.checks']) || ! empty($perms['accounting']), 'active' => str_contains($path, 'staff/accounting/checks') && ! str_contains($path, 'checks/')],
