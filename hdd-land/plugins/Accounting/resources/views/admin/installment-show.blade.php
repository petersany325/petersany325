...
<div class="top">
  ...
  <div class="actions">
    <a class="btn g" href="{{ url('/admin/accounting/installments') }}">بازگشت</a>
  </div>
</div>
...
<div class="panel"><div class="hd"><strong>جزئیات</strong></div><div class="bd">
  ...
  @if(!empty($invoice))
    <p><strong>فاکتور فروش:</strong> <a href="{{ url('/admin/accounting/docs/'.$invoice->id) }}">{{ $invoice->number }}</a> — {{ $invoice->status }}</p>
  @endif
...
<form method="post" action="{{ url('/admin/accounting/installments/'.$row->id.'/status') }}" class="form">@csrf
  <div class="row">
    <label>وضعیت
      <select name="status">
...
<table>
  ...
  @forelse($schedules as $s)
    <tr>
      ...
      <td>
        <form method="post" action="{{ url('/admin/accounting/installments/'.$row->id.'/schedules/'.$s->id) }}" style="display:flex;gap:.35rem;align-items:center">
          @csrf
...
<form method="post" action="{{ url('/admin/accounting/installments/'.$row->id.'/delete') }}" onsubmit="return confirm('حذف کامل؟')">
  @csrf
