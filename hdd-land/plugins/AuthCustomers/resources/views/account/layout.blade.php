...
<style>
  ...
  @media(max-width:850px){
    .cab-shell{grid-template-columns:1fr;margin-top:14px}
    .cab-side{
      position:fixed;top:0;right:0;bottom:0;z-index:80;width:min(300px,88vw);
      display:none;overflow:auto;border-radius:0;box-shadow:-18px 0 40px rgba(15,23,42,.28)
    }
    .cab-shell.menu-open .cab-side{display:block}
    .cab-shell.menu-open:before{content:"";position:fixed;inset:0;z-index:70;background:rgba(15,23,42,.45)}
    .cab-mobile-menu{display:inline-flex}
    .cab-topbar{position:sticky;top:7px;z-index:20}
    .cab-main{grid-row:2}
    .cab-shell.menu-open .cab-main{grid-row:auto}
  }
</style>
...
<script>(function(){var b=document.querySelector('.cab-mobile-menu'),s=document.querySelector('#customerCabinet');if(!b||!s)return;b.addEventListener('click',function(){s.classList.toggle('menu-open')});s.addEventListener('click',function(e){if(e.target===s && s.classList.contains('menu-open')) s.classList.remove('menu-open')});document.querySelectorAll('.cab-links a').forEach(function(a){a.addEventListener('click',function(){if(window.innerWidth<=850)s.classList.remove('menu-open')})})})();</script>
@endsection
