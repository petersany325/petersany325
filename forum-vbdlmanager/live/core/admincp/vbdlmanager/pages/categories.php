...
if ($action === 'edit' || $action === 'add')
{
	...
	$cat = $categoryid ? $repo->getCategory($categoryid) : array('title' => '', 'description' => '', 'displayorder' => 0, 'active' => 1);
	...
	echo '<div class="vbdl-form-row"><label>Default access</label><div>';
	foreach ($groups as $g)
	{
...
echo '<table class="vbdl-table" style="margin-top:14px"><tr><th>ID</th><th>Title</th><th>Order</th><th>Active</th><th></th></tr>';
if (empty($cats))
{
	echo '<tr><td colspan="5" class="vbdl-muted">No categories yet.</td></tr>';
}
foreach ($cats as $c)
{
	echo '<tr><td>' . (int)$c['categoryid'] . '</td><td>' . vbdl_h($c['title']) . '</td><td>' . (int)$c['displayorder'] . '</td>';
