...
echo '<table class="vbdl-table" style="margin-top:14px"><tr><th>ID</th><th>Title</th><th>Mode</th><th>Files</th><th>Order</th><th>Active</th><th></th></tr>';
if (empty($cats))
{
	echo '<tr><td colspan="7" class="vbdl-muted">No categories yet.</td></tr>';
}
foreach ($cats as $c)
{
	...
	echo '<tr><td>' . $cid . '</td><td>' . vbdl_h($c['title']) . '</td><td>' . vbdl_h($mode) . '</td>';
	echo '<td><a href="' . vbdl_h(vbdl_admin_url('files', 'categoryid=' . $cid)) . '">' . $cnt . ' file' . ($cnt === 1 ? '' : 's') . '</a></td>';
	echo '<td>' . (int)$c['displayorder'] . '</td>';
	echo '<td>' . (!empty($c['active']) ? '<span class="vbdl-chip ok">Yes</span>' : '<span class="vbdl-chip no">No</span>') . '</td>';
	echo '<td><a href="' . vbdl_h(vbdl_admin_url('categories', 'action=edit&categoryid=' . $cid)) . '">Edit</a> ';
	echo '<form style="display:inline" method="post" action="admincp/vbdlmanager.php" onsubmit="return confirm(\'Delete category?\');">';
	vbdl_request_token_field();
	echo '<input type="hidden" name="do" value="categories" /><input type="hidden" name="action" value="delete" /><input type="hidden" name="categoryid" value="' . $cid . '" /><button class="vbdl-btn danger" type="submit">Delete</button></form></td></tr>';
}
echo '</table></div></div>';
vbdl_admin_footer();
