...
@media (max-width:720px){.table .hide-sm{display:none}.btn{width:100%}}
...
else
{
	...
	// vB6: dedicated /login routes show Invalid Page URL; use forum home header Login dropdown.
	echo '<a class="btn secondary" href="../" title="Open forum home and use Login or Sign Up">Sign in</a>';
}
...
if ($view === 'account')
{
	...
	if ($userid < 1)
	{
		...
		echo '<p><a class="btn" href="../" title="Open forum home and use Login or Sign Up">Sign in to forum</a></p>';
		echo '<p class="meta">On the forum home, open <strong>Login or Sign Up</strong> in the header, then return here to Downloads.</p>';
	}
	else
	{
	...
	echo '</div></div><div class="status"><span>User account cartable</span><span>' . vbdl_fe_h($username) . '</span></div></div></div></body></html>';
	exit;
}

if (!$canView)
{
	echo '<div class="panel"><h1>Downloads</h1><p class="denied">You do not have permission to view the Downloads section.</p>';
	if ($userid < 1)
	{
		echo '<p><a class="btn" href="../">Sign in to forum</a></p>';
	}
	echo '</div>';
