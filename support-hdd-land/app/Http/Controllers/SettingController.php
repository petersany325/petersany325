...
class SettingController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('settings.index', [
            ...
            'receiptPrefix' => Reception::receiptPrefix(),
            'receiptPrefixSample' => Reception::receiptPrefix().'1000',
    ...
    public function updateGeneral(Request $request)
    {
        $data = $request->validate([
            ...
            'receipt_prefix' => ['nullable', 'string', 'max:12', 'regex:/^[A-Za-z0-9\-]*$/'],
        ], [
            'receipt_prefix.regex' => 'پیشوند قبض فقط حروف انگلیسی، عدد و خط تیره مجاز است.',
        ]);
        ...
        $prefix = Reception::normalizeReceiptPrefix((string) ($data['receipt_prefix'] ?? ''));
        AppSetting::setValue('receipt_prefix', $prefix);

        return $this->settingsRedirect($request, 'general', 'success', 'تنظیمات عمومی و پیشوند قبض ذخیره شد.');
    }
