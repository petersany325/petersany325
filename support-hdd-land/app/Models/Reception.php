...
class Reception extends Model
{
    ...
    /**
     * Per-install receipt prefix (e.g. T-20N for one shop, R for others).
     * Never hardcode a single shop brand into all customer installs.
     */
    public static function receiptPrefix(): string
    {
        $configured = trim((string) AppSetting::getValue('receipt_prefix', ''));
        if ($configured !== '') {
            return self::normalizeReceiptPrefix($configured);
        }

        // Legacy installs that already issued T-20N receipts keep that prefix
        // until an admin sets a custom value in Settings.
        if (static::withTrashed()->where('receipt_no', 'like', 'T-20N%')->exists()) {
            return 'T-20N';
        }
    ...
    public static function normalizeReceiptPrefix(string $prefix): string
    {
        $prefix = trim($prefix);
        // Allow Latin letters, digits, hyphen — customer-facing receipt codes.
        $prefix = preg_replace('/[^A-Za-z0-9\-]/', '', $prefix) ?? '';
        $prefix = strtoupper(substr($prefix, 0, 12));
